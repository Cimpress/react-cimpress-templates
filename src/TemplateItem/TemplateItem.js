...
class TemplateItem extends React.Component {
    ...
    componentDidMount() {
        if (this.props.template && this.props.template.templateId) {
            this.getGroup();
        }
    }
    ...
    componentDidUpdate(prevProps) {
        if (this.props.template && this.props.template.templateId) {
            if (!prevProps.template || (prevProps.template.templateId !== this.props.template.templateId)) {
                this.getGroup();
            }
        }
    }
    ...
    getGroup() {
        return getTemplateAdminGroup(this.props.accessToken, this.props.template.templateId)
    ...
    onTemplateCreated(dialogOpenFlagName, newTemplateId) {
        this.setState({
            [dialogOpenFlagName]: false,
        });

        if (this.props.onTemplateCreated) {
            this.props.onTemplateCreated(newTemplateId);
        }

        if (this.props.autoRedirectAfterCreation) {
            window.open(this._templateDesignerHref(newTemplateId), '_blank');
        }
    }
    ...
    renderCreateTemplateDialog() {
        return <NewTemplateModal
            ...
            prefixTemplateAtCreation={this.props.autoPrefixTemplateWhenCreating ? this.props.templateSelectionPrefix : undefined}
            accessToken={this.props.accessToken}
            oldTemplateId={this.props.template.templateId}
            open={this.state.openDialogCreateTemplate}
            onCancel={() => this.onCancel('openDialogCreateTemplate')}
            onConfirm={(oldTemplateId, newTemplateId) => {
                this.onTemplateCreated('openDialogCreateTemplate', newTemplateId);
            }}
        />;
    }
            ...
            header={
                <div>
                    ...
                    <h5 style={{textAlign: 'left'}}>
                        <span className={'active-template-name-full'}>{this.props.template.title}</span>
                    </h5>
                </div>
            }
    ...
    render() {
        ...
        const title = <span className={'rct'}>
                ...
                <span className={'template-name'}>
                    ...
                    {this.props.withComments
                        ? <CommentsDrawerLink
                            ...
                            resourceUri={`https://stereotype.trdlnk.cimpress.io/v1/templates/${item.templateId}`}
                            newestFirst
                            editComments
                            accessToken={this.props.accessToken}/>
                        : null}
                ...
                {this.props.withEditButton
                    ? <Fragment>
                        ...
                        <IconButton
                            ...
                            href={this._templateDesignerHref(item.templateId)}
                            target={'_blank'}
